...
def load_data(path='./data/train'):
    ...
    # 存储25000个文件中的review 通过预处理每个review以string的形式放入一个data list中
    # len(data) = 25000 ==> ['review1', 'review2', ......]
    return data
...
def load_glove_embeddings():
    ...
    # embeddings（glove_array): glove里每一个word的词向量 添加到该list里 [ [50个0 代表 Unknown], [50个数字 代表某个word]]
    #     glove里总共有40w个单词 + 1 Unknwon==> len(embeddings) = 400001
    # word_index_dict(glove_dict): 将glove里的每一个word按顺序映射到dict里 {'UNK':0, 'word1':1... }
    #     glove里总共有40w个单词 + 1 UNK==> len(word_index_dict) = 400001
...
def embedd_data(training_data_text, e_arr, e_dict):
    ...
    """
    num_samples = len(training_data_text) # 25000
    embedded = np.zeros([num_samples, MAX_WORDS_IN_REVIEW, EMBEDDING_SIZE]) # [25000, maxwr, embsize]
    ...
    # e_arr(embeddings == glove_array)
    # e_dict(word_index_dict == glove_dict)
    # embedded 通过双层循环 将data里 25000个review 按照每个word依次记录在review_mat里 再将review_mat填写到embedded中
    # e_dict.get(word, 0) ==> 有该word 便返回该word的index 否则返回0
    # e_arr[index] ==> 根据字典中得到的index 返回该word的词向量（50个数字代表的词向量）
    # review_mat[w] ==> 记录1个review文件里 所有word的词向量 shape=[maxwr, embsize]
    # embedded[i] = review_mat ==> 记录25000个review文件的词向量 shape=[25000, maxwr, embsize]
    return embedded
