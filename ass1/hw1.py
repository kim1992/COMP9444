...
def my_perceptron(x):
    ...
    """
    i = tf.placeholder(tf.float32, shape=[x], name='i')
    j = tf.reshape(i, [x, 1])
    w = tf.get_variable(name='w', shape=[1, x], initializer=tf.ones_initializer())
    mul_sum = tf.matmul(w, j)
    out = my_relu(mul_sum)
    return i, out
...
def onelayer(X, Y, layersize=10):
    ...
    """
    w = tf.Variable(tf.zeros([int(X.shape[1]), layersize]))
...
#        kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))
    ...
    x_flatten = tf.reshape(conv2, [-1, int(conv2.shape[1]) * int(conv2.shape[2]) * convlayer_sizes[1]])
    w = tf.Variable(tf.truncated_normal([int(x_flatten.shape[1]), outputsize], stddev=0.1))
    b = tf.Variable(tf.zeros([outputsize]))
    logits = tf.matmul(x_flatten, w) + b
    preds = tf.nn.softmax(logits)

    # batch_xentropy = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=Y)
    # batch_xentropy = tf.nn.softmax_cross_entropy_with_logits_v2(logits=logits, labels = tf.stop_gradient(Y))
    batch_xentropy = -tf.reduce_sum(Y * tf.log(preds))
    batch_loss = tf.reduce_mean(batch_xentropy)
